...
/**
 * NOTEBOOK.md の初期テンプレート。
 *
 * これはプラグインが強制する規範ではなく、ユーザーが所有する層の初期値である。
 * 自動生成では二度と上書きされないので、不要なら消してよいし、自由に書き換えてよい。
 * 「振る舞いを縛る指示」をプラグイン側（CLAUDE.md / システムプロンプト）に置かず、
 * ここに置く理由は、ユーザーが見えて・編集できて・消せる場所だからである。
 */
const NOTEBOOK_MD_TEMPLATE = `# このノートブック固有の指示

<!-- このファイルは自動生成されません。自由に編集・削除してください。
     CLAUDE.md から読み込まれ、エージェントの作業方針になります。 -->
