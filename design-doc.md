...
### 4.2 作業の進め方は `NOTEBOOK.md` で表現する

「まず計画を出させて、レビューしてから実装させたい」という運用は、**プラグイン側の機構では表現しません。** ノートブックごとの `NOTEBOOK.md` に書き、CLI に読ませます。

`NOTEBOOK.md` の初期テンプレートには次を入れてあります。ユーザーが所有する層なので、編集も削除も自由です。

```markdown
## 進め方
- 新しい成果物の作成を指示されたときは、いきなり書き始めず、まず構成案
  （章立て・使うソース・判断が必要な論点）をチャットに示してレビューを受けること。
- 合意が取れてから artifacts/ にファイルを作成する。
- ただし「すぐ作って」「このまま進めて」等と明示された場合は、確認せずに作成してよい。
- 既存成果物の部分修正や、方針が明確な作業は、確認なしで進めてよい。
```

自動生成の `CLAUDE.md` ではなく `NOTEBOOK.md` に置くのが要点です。`CLAUDE.md` は毎回上書きされるプラグインの所有物で、そこに規範を書くのは「隠れたプロンプト注入」に戻ります。`NOTEBOOK.md` は**見えて・編集できて・消せる**ので、ユーザーの設定であってラッパーの強制ではありません。

> **⚠️ 実行モードの UI を再導入してはいけない**
>
> 一度「相談 / 作成」のトグルを実装し、相談モードを `--disallowedTools Write,Edit,Bash` で
> 実現したが、**設計そのものが間違っていた**。
>
> - 「Write を禁止する」は「計画を先に出させる」の実装になっていない。
>   書けなくなるだけで、計画が出るとは限らない
> - 既定が相談モードだったため、ユーザーが「見積を作成して」と打っても
>   AI が「Write が使えない」と言って失敗した。ユーザーの意図（文章）と
>   モード（別のスイッチ）が食い違う、典型的なフットガン
> - `--resume` で会話が継続する以上、「まず計画を」「ではこれで作って」は
>   ただの会話で成立する。モードという概念自体が不要だった
>
> **振る舞いの傾向づけをツール制限や権限で表現しないこと。** それは能力の剥奪であって、
> 方針の伝達ではない。方針は `NOTEBOOK.md` に書く。

> **⚠️ `--permission-mode plan` も使ってはいけない**
>
> plan モードは `ExitPlanMode` の承認を人間に求める対話前提のモードで、`-p`（非対話）では
> 応答手段がない。実測では権限確認と `Exit plan mode?` が繰り返し記録され、
> 計画も成果物も得られずに終了した。stdin を開いたまま `y` を流す回避策も、
> 権限プロンプトが TTY 前提のため無効。
>
> **権限モードは常に `bypassPermissions`**（旧版は `--dangerously-skip-permissions`）にして、
> 承認プロンプトが発生しうる経路そのものを潰す。
