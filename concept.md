...
### 作業の進め方は NOTEBOOK.md に書く

「まず計画を出させて、レビューしてから実装させる」という運用は、プラグインの機構では表現しません。ノートブックごとの `NOTEBOOK.md`（人間が所有し、自動生成では上書きされない層）に書き、CLI に読ませます。初期テンプレートにその方針を入れてありますが、編集も削除も自由です。

一度「相談 / 作成」のモード切替 UI を実装し、相談モードを「書き込みツールを外す」ことで実現しましたが、これは誤りでした。**ツールを取り上げることは、能力の剥奪であって方針の伝達ではありません。** 書けなくしても計画が出るとは限らず、実際には「作成して」と打ったユーザーに対して AI が「Write が使えない」と答えるだけの結果になりました。

`--resume` で会話が継続する以上、「まず計画を」「ではそれで作って」はただの会話で成立します。モードという概念自体が不要でした。
